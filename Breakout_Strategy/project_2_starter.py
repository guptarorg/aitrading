...
def get_high_lows_lookback(high, low, lookback_days):
    ...
    """
    lookback_high = high.rolling(lookback_days).max()
    lookback_low = low.rolling(lookback_days).min()
    #TODO: Implement function
    return lookback_high.shift(1), lookback_low.shift(1)
...
def get_long_short(close, lookback_high, lookback_low):
    ...
    """
    #TODO: Implement function
    long_pos = close>lookback_high
    short_pos = close<lookback_low
    long_short = long_pos*1 + short_pos*-1
...
def filter_signals(signal, lookahead_days):
    ...
    for i in range(0,len(columns)):
        single_stock = pd.Series(filtered_signal[columns[i]])
        long_signal = (single_stock==1)*1
        short_signal = (single_stock==-1)*-1
        long_pos = clear_signals(long_signal, lookahead_days)
        short_pos = clear_signals(short_signal, lookahead_days)
        total_pos = long_pos + short_pos
        filtered_signal[columns[i]] = total_pos
...
# Range of returns (i.e. variance of returns) increases as the lookahead_days is increased, it also leads to fatter tails. 
...
def calculate_kstest(long_short_signal_returns):
    ...
    """
    #TODO: Implement function
    port_mean = np.mean(long_short_signal_returns)
    port_std = np.std(long_short_signal_returns, ddof = 0)
    normal_args = (port_mean, port_std)
    ks_values = pd.Series()
    p_values = pd.Series()
    grouped_by = long_short_signal_returns.groupby('ticker')
    for name, group in grouped_by:
        sample = group['signal_return'].values
        test_ks, test_p = kstest(sample,'norm',normal_args)
        ks_values[name] = test_ks
        p_values[name] = test_p
...
def find_outliers(ks_values, p_values, ks_threshold, pvalue_threshold=0.05):
    ...
    return set(ks_values[outliers].index)
